...
const MODULES = [
  ...
  },
  {
    ...
    fields: [
      ["Source of WO", "ENUM", "Yes", "CMT, CSA, CPO, CBA, CST, CSO, CGI, CSF, Legal, Procurement, SVP, Chairman, CSI HOD, Others"],
  ...
  },
  {
    ...
    status: "NOT STARTED",
    description: "Staff profiles, certifications tracking, skill matrix, and skills gap analysis for the CSI department.",
    workflow: [
      "Admin maintains staff profiles with skills and certifications",
      "Certifications tracked with expiry dates and renewal alerts",
      "Skill matrix shows team capabilities across domains",
      "Gap analysis identifies missing skills vs project requirements",
    ],
    fields: [
      ["Certification Name", "VARCHAR(100)", "Yes", "Name of certification"],
      ["Issued Date", "DATE", "Yes", "When obtained"],
      ["Expiry Date", "DATE", "No", "When it expires"],
      ["Skill Category", "VARCHAR(50)", "Yes", "Domain grouping"],
      ["Proficiency Level", "ENUM", "Yes", "Beginner / Intermediate / Expert"],
    ],
    endpoints: [
      ["GET", "/api/skills", "Any", "Staff skills matrix (planned)"],
      ["GET", "/api/certifications", "Any", "Certification list (planned)"],
    ],
    roleMatrix: [
      ["View Skills", "Yes", "Yes", "Yes", "Yes"],
      ["Manage Skills", "Yes", "No", "No", "No"],
    ],
    rules: [
      "STAFF rows are never hard-deleted (Active/Inactive/OnLeave status)",
      "Certification expiry warnings at 30/60/90 days",
    ],
    notes: "Not yet started. Database tables exist from initial migration.",
  },
  {
    num: 15,
    name: "KPI & Evidence Dashboard",
    status: "NOT STARTED",
    description: "KPI targets vs actuals tracking with evidence linkage for quarterly/annual performance reporting.",
    workflow: [
      "HOD sets KPI targets per period",
      "Staff link evidence uploads to KPI items",
      "Dashboard shows target vs actual with visual indicators",
      "Quarterly and annual report generation",
    ],
    fields: [
      ["KPI Name", "VARCHAR(100)", "Yes", "Performance indicator name"],
      ["Target Value", "NUMERIC", "Yes", "Expected target"],
      ["Actual Value", "NUMERIC", "No", "Measured result"],
      ["Period", "VARCHAR(20)", "Yes", "Q1/Q2/Q3/Q4/Annual"],
      ["Evidence Links", "FK → EVIDENCE", "No", "Supporting documents"],
    ],
    endpoints: [
      ["GET", "/api/kpi", "HOD/SM", "KPI dashboard (planned)"],
    ],
    roleMatrix: [
      ["View KPIs", "Yes", "Yes", "No", "No"],
      ["Set Targets", "Yes", "No", "No", "No"],
      ["Link Evidence", "Yes", "Yes", "Yes", "Yes"],
    ],
    rules: [
      "KPI targets set by HOD only",
      "Evidence linkage available to all staff",
    ],
    notes: "Not yet started. Placeholder page exists.",
  },
  {
    num: 16,
    name: "Reports & Analytics",
    status: "PARTIALLY BUILT",
    description: "Report generation and export capabilities for WO data, utilization, and performance metrics.",
    workflow: [
      "User selects report type and date range",
      "System generates report from live data",
      "Export to PDF or Excel (planned)",
    ],
    fields: [],
    endpoints: [
      ["GET", "/api/reports", "HOD/SM", "Generate report data"],
    ],
    roleMatrix: [
    ...
    notes: "Basic report endpoint exists. Export functionality and additional report types planned.",
  },
  {
    num: 17,
    name: "Admin Panel",
    status: "COMPLETED",
    description: "Administrative panel for approving staff registrations. Accessible only to users with SystemConfigFlag=true.",
    workflow: [
      "Admin navigates to Admin panel",
      "Views pending registration requests",
      "Reviews staff details: name, email, staff code, department, role",
      "Approves or rejects each request",
      "Approved staff can then request OTP and login",
    ],
    fields: [],
    endpoints: [
    ...
    rules: [
      "Only users with SystemConfigFlag=true can access",
      "Approval is required before first login",
    ],
    notes: "Fully implemented. *Requires SystemConfigFlag, not role-based.",
  },
  {
    num: 18,
    name: "Notifications",
    status: "NOT STARTED",
    description: "In-app notification system for WO assignments, SLA warnings, and approval requests.",
    workflow: [
      "System generates notification on trigger events",
    ...
    fields: [
      ...
      ["Category", "ENUM", "Yes", "WorkOrder / Approval / SLA / System"],
      ["Link URL", "VARCHAR(300)", "No", "Navigation target"],
      ["Read At", "TIMESTAMP", "No", "When user read the notification"],
    ],
    endpoints: [
      ["GET", "/api/notifications", "Any", "List notifications (planned)"],
      ["PATCH", "/api/notifications/:id/read", "Owner", "Mark as read (planned)"],
    ],
    roleMatrix: [
    ...
    rules: [
      ...
      "Notification bell count badge is built (shows unread count)",
    ],
    notes: "Infrastructure partially built (notification count badge, create function). Full notification list UI and mark-as-read not yet implemented.",
  },
  {
    num: 19,
    name: "Audit Trail",
  ...
  },
  {
    ...
    status: "NOT STARTED",
    description: "Administrative interface for managing lookup tables (request types, complexity tiers, departments), system settings, and role permissions.",
    workflow: [
      "Admin navigates to system configuration",
      "Manage lookup tables: add/edit request types, tiers, departments",
      "Configure system settings",
      "View and manage role permissions",
    ],
    fields: [],
    endpoints: [
      ["GET", "/api/lookups", "Any", "Read all lookup tables (exists)"],
    ],
    roleMatrix: [
      ["View Configuration", "Yes*", "No", "No", "No"],
      ["Modify Configuration", "Yes*", "No", "No", "No"],
    ],
    rules: [
      "Only users with SystemConfigFlag=true",
      "Changes to lookup tables should be audited",
    ],
    notes: "Not yet started. Read-only lookups API exists. Management UI planned.",
  },
];
...
async function main() {
    ...
    sections: [
      {
          ...
          default: new Header({
            children: [new Paragraph({
              ...
              children: [
                ...
                font("  |  v2.0  |  INTERNAL USE ONLY", { size: 16, color: GRAY }),
              ],
            })],
          }),
        },
        ...
        children: [
          ...
          // ── Key Changes in v2.0 ──
          pageBreak(),
          heading1("Key Changes in v2.0"),
          bodyText("Major updates since v1.0 (25 June 2026):"),
          new Paragraph({ spacing: { after: 60 } }),

          ...[
            "Work Order Module redesigned: dual priority (inter-dept + internal), SLA in working days, 14 WO sources, monitoring staff, optional assignment at creation, task checklist with % progress",
            "WO Task Checklist added: per-WO child tasks with 0–100% progress tracking, auto-populated 16-item template for Tender/RFP WOs",
            "Tender Module clarified: CSI has read-only view; CMT owns intake/commercial/submission; Go/No-Go by committee (HOD CSI + HOD CMT)",
            "Multi-department platform: CSIDOP serves CSI (now) and CMT (later) with department-scoped sidebar menus and data",
            "7 new departments added: CST, CSO, CSF, Legal, Procurement, SVP, Chairman",
            "Sidebar reorganized: Tenders moved to bottom as read-only link; ‘+ New Tender’ button removed from Workloads page",
            "Optional WO assignment at creation: ‘Assign To Staff’ section on create form with assignee picker and hours input",
          ].map((text) =>
            new Paragraph({
              numbering: { reference: "bullets", level: 0 },
              spacing: { after: 60 },
  ...
  fs.writeFileSync("C:\\Projects\\csi-digital-operating-platform\\docs\\CSIDOP_Module_Workflow_Review_v2.docx", buffer);
  console.log("Generated CSIDOP_Module_Workflow_Review.docx (v2.0)");
}
